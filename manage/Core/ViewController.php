...
class ViewController
{
    ...
    public function __construct ( string $content = '', array $variables = array(), array $replacements = array() )
    {
        ...
        # Ensure we have required minimum set of variables for the template
        if ( !isset($this->variables['pagetitle']) ) { $this->variables['pagetitle'] = ''; }    // Title of the page
        if ( !isset($this->variables['sitename']) ) { $this->variables['sitename'] = $CONFIG->getSetting( 'Core', 'sitename' ); }
        if ( !isset($this->variables['language']) ) { $this->variables['language'] = $LANG->language; }
    }
    ...
    public function renderOutput ()
    {
        ...
        require_once ( DOCROOT . 'Themes/default/' . $this->template . '.php' );
        ob_end_flush();
    }
    ...
    {
        return $this->template = (string) $templateName ?: 'template';
    }
